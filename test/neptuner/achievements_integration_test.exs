defmodule Neptuner.AchievementsIntegrationTest do
  ...
  describe "full achievement lifecycle" do
    test "user progresses through achievement completion cycle" do
      ...
      assert {:ok, user_achievement} = Achievements.create_or_update_user_achievement(user.id, "test_achievement", 3)
      ...
      assert {:ok, user_achievement} = Achievements.create_or_update_user_achievement(user.id, "test_achievement", 8)
      assert user_achievement.progress_value == 8
      assert is_nil(user_achievement.completed_at)

      # Final progress - achievement completed
      assert {:ok, user_achievement} = Achievements.create_or_update_user_achievement(user.id, "test_achievement", 15)
      assert user_achievement.progress_value == 15
      assert user_achievement.completed_at
      assert is_nil(user_achievement.notified_at)

      # Mark as notified
      assert {:ok, user_achievement} = Achievements.mark_achievement_notified(user.id, "test_achievement")
    ...
    test "achievement completion with nil threshold_value" do
      ...
      assert {:ok, user_achievement} = Achievements.create_or_update_user_achievement(user.id, "instant_achievement", 1)
    ...
    test "multiple users progressing on same achievement" do
      ...
      assert {:ok, ua1} = Achievements.create_or_update_user_achievement(user1.id, "shared_achievement", 3)
      
      # User 2 makes different progress
      assert {:ok, ua2} = Achievements.create_or_update_user_achievement(user2.id, "shared_achievement", 7)
      ...
      assert ua2.completed_at  # User 2 completed it
    end
    ...
    test "achievement statistics update correctly as user progresses" do
      ...
      assert stats.in_progress == 0
      assert stats.completion_percentage == 33  # 1/3 * 100
  ...
  describe "achievement querying and filtering" do
    test "list_user_achievements with various filters" do
      ...
      completed_task_ua = insert(:completed_user_achievement, 
        user: user, 
        achievement: task_achievement,
        completed_at: DateTime.utc_now() |> DateTime.add(-1, :day)
      )
      
      in_progress_habit_ua = insert(:user_achievement, 
        user: user, 
      ...
      minimal_achievements = Achievements.list_user_achievements(user.id, include_achievement: false)
    ...
    test "achievement listing with category and active filters" do
      ...
      all_task_achievements = Achievements.list_achievements(category: "tasks", active_only: false)
  ...
  describe "edge cases and error handling" do
    test "handles concurrent updates to same user achievement" do
      ...
      assert final_ua.progress_value >= 6  # At least one update succeeded
    end
  ...
  describe "data consistency and relationships" do
    test "user achievement unique constraint enforcement" do
      ...
      assert {:ok, _} = Repo.insert(%UserAchievement{
        user_id: user.id,
    ...
    test "achievement key unique constraint enforcement" do
      ...
      assert {:ok, _} = Achievements.create_achievement(%{
        key: "unique_test",
