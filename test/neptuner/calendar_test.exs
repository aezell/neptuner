defmodule Neptuner.CalendarTest do
  ...
  describe "create_meeting/3" do
    ...
    test "returns error with invalid attrs" do
      ...
      attrs = %{title: ""}  # Missing required scheduled_at
  ...
  describe "upsert_meeting_by_external_id/4" do
    test "creates new meeting when external_id doesn't exist" do
      ...
      assert {:ok, meeting} = Calendar.upsert_meeting_by_external_id(
        user.id, service_connection.id, external_id, attrs
      )
    ...
    test "updates existing meeting when external_id exists" do
      user = insert(:user)
      external_id = "ext_123"
      existing_meeting = insert(:meeting, user: user, external_calendar_id: external_id, title: "Old Title")
      service_connection = insert(:service_connection, user: user)
      
      attrs = %{title: "Updated Title", duration_minutes: 60}

      assert {:ok, updated_meeting} = Calendar.upsert_meeting_by_external_id(
        user.id, service_connection.id, external_id, attrs
      )
  ...
  describe "get_meetings_needing_rating/1" do
    test "returns unrated meetings older than yesterday" do
      ...
      old_unrated = insert(:unrated_meeting, 
        user: user, 
        scheduled_at: DateTime.utc_now() |> DateTime.add(-2, :day)
      )
      
      _old_rated = insert(:rated_meeting, 
        user: user,
        scheduled_at: DateTime.utc_now() |> DateTime.add(-2, :day)
      )
      
      _recent_unrated = insert(:unrated_meeting,
        user: user,
    ...
    test "orders by scheduled_at desc (most recent first)" do
      ...
      older = insert(:unrated_meeting,
        user: user,
        scheduled_at: DateTime.utc_now() |> DateTime.add(-3, :day)
      )
      
      newer = insert(:unrated_meeting,
        user: user,
        scheduled_at: DateTime.utc_now() |> DateTime.add(-2, :day)
      )
  ...
  describe "get_meeting_statistics/1" do
    test "calculates comprehensive meeting statistics" do
      ...
      insert(:could_have_been_email_meeting, user: user, duration_minutes: 60, meeting_type: :status_update)
      insert(:meeting, user: user, duration_minutes: 90, actual_productivity_score: 8, could_have_been_email: false, meeting_type: :other)
      insert(:standup_meeting, user: user, duration_minutes: 30, actual_productivity_score: 6, could_have_been_email: false)
      ...
      assert stats.could_have_been_email_count == 2  # status_update + all_hands 
      assert stats.total_hours_in_meetings == 4.5    # 270 minutes / 60
      assert stats.average_productivity_score == 7.0  # (8 + 6) / 2
      ...
      assert stats.other_meetings == 1  # the explicitly created :other meeting
    end
    ...
    test "handles meetings with nil duration" do
      ...
      assert stats.total_hours_in_meetings == 1.0  # Only counts the 60-minute meeting
    end
  end
  ...
  describe "get_weekly_meeting_report/1" do
    test "generates weekly report for meetings" do
      ...
      _this_week_1 = insert(:this_week_meeting, 
        user: user, 
        duration_minutes: 60,
        attendee_count: 5,
      ...
      _this_week_2 = insert(:this_week_meeting,
        user: user,
        duration_minutes: 30,
        attendee_count: 3,
      ...
      assert report.week_hours == 1.5  # 90 minutes / 60
      assert report.week_attendees == 8
      assert report.collective_human_hours_lost == 5.0  # 1 hour * 5 attendees
    end
  ...
  describe "sync_meeting_from_external/3" do
    test "creates new meeting from external data" do
      ...
      assert {:ok, meeting} = Calendar.sync_meeting_from_external(user.id, service_connection.id, external_data)
      ...
      assert meeting.meeting_type == :status_update  # Classified from "sync" in title
    end
    ...
    test "updates existing meeting from external data" do
      ...
      existing_meeting = insert(:meeting,
        user: user,
      ...
      assert {:ok, updated_meeting} = Calendar.sync_meeting_from_external(user.id, service_connection.id, external_data)
    ...
    test "classifies meeting types correctly" do
      ...
      for {title, expected_type} <- test_cases do
        ...
        assert {:ok, meeting} = Calendar.sync_meeting_from_external(user.id, service_connection.id, external_data)
        assert meeting.meeting_type == expected_type, "Title '#{title}' should classify as #{expected_type}"
      end
    end
