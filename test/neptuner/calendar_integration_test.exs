defmodule Neptuner.CalendarIntegrationTest do
  ...
  describe "complete calendar sync workflow" do
    test "syncs meetings from multiple providers and generates insights" do
      ...
      assert recommendations.meeting_load_status in [:light_load, :moderate_load, :heavy_load, :meeting_overload]
      assert recommendations.email_worthiness_status in [:meeting_efficient, :moderately_efficient, :email_heavy]
    end
    ...
    test "handles meeting rating workflow" do
      ...
      assert stats.average_productivity_score == 6.0  # (8 + 4) / 2

      # Verify fewer meetings need rating now
      remaining_unrated = Calendar.get_meetings_needing_rating(user.id)
      assert length(remaining_unrated) == 0  # Future meeting shouldn't need rating yet
    end
    ...
    test "handles meeting type classification and filtering" do
      ...
      _one_on_one = insert(:one_on_one_meeting, user: user, service_connection: service_connection)
      _brainstorm = insert(:brainstorm_meeting, user: user, service_connection: service_connection)
    ...
    test "handles date range queries across different time periods" do
      ...
      june_meeting = insert(:meeting, 
        user: user, 
        service_connection: service_connection,
        scheduled_at: base_time
      )
      
      july_meeting = insert(:meeting,
        user: user,
        service_connection: service_connection, 
        scheduled_at: DateTime.add(base_time, 20, :day)
      )
      
      august_meeting = insert(:meeting,
        user: user,
        service_connection: service_connection,
      ...
      cross_month_meetings = Calendar.list_meetings_for_date_range(user.id, cross_month_start, cross_month_end)
    ...
    test "handles external meeting sync with updates" do
      ...
      assert {:ok, meeting} = Calendar.sync_meeting_from_external(
        user.id, 
      ...
      assert {:ok, updated_meeting} = Calendar.sync_meeting_from_external(
        user.id,
        service_connection.id,
        updated_external_data
      )
    ...
    test "generates comprehensive insights based on meeting patterns" do
      ...
      for i <- 1..30 do
        insert(:meeting,
          ...
          duration_minutes: 60,
          attendee_count: 8,
          could_have_been_email: i <= 22,  # 73% could have been email
      ...
      assert stats.average_productivity_score < 5.0  # Low productivity scores

      # Verify weekly report captures the pattern
      assert weekly_report.week_meetings == 30
      assert weekly_report.week_hours == 30.0
      assert weekly_report.collective_human_hours_lost > 100  # 22 meetings * 8 attendees * 1 hour
    ...
    test "handles multi-user isolation correctly" do
      ...
      user1_meeting = insert(:meeting, user: user1, service_connection: service_connection1, title: "User 1 Meeting")
      user2_meeting = insert(:meeting, user: user2, service_connection: service_connection2, title: "User 2 Meeting")
    ...
    test "handles edge cases in meeting data" do
      ...
      _minimal_meeting = insert(:meeting,
        user: user,
        service_connection: service_connection,
        title: "M",  # Single character
      ...
      _maximal_meeting = insert(:meeting,
        user: user,
        service_connection: service_connection,
        title: String.duplicate("A", 250),  # Database varchar limit is likely 255
      ...
      assert stats.total_hours_in_meetings == 8.0  # Only maximal meeting counts

      # Should handle mixed productivity scores
      assert stats.average_productivity_score == 10.0  # Only maximal meeting is rated
    ...
    test "performance with large datasets" do
      ...
      _meetings = for i <- 1..meeting_count do
        insert(:meeting,
          user: user,
          service_connection: service_connection,
          scheduled_at: DateTime.utc_now() |> DateTime.add(-i, :hour),
      ...
      recent_meetings = Calendar.list_meetings_for_date_range(user.id, week_ago, DateTime.utc_now())
  ...
  describe "error handling and edge cases" do
    ...
    test "handles database constraints appropriately" do
      ...
      invalid_attrs = %{
        ...
        duration_minutes: -30,  # Must be positive
        attendee_count: -5,  # Must be non-negative
        actual_productivity_score: 15,  # Must be 1-10
        meeting_type: :invalid_type,  # Must be valid enum
        scheduled_at: "invalid_date"  # Must be valid datetime
      }

      assert {:error, changeset} = Calendar.create_meeting(user.id, service_connection.id, invalid_attrs)
    ...
    test "handles concurrent operations safely" do
      ...
      unique_external_ids = final_meetings
