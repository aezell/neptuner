defmodule Neptuner.Factory do
  ...
  def habit_factory do
    %Habit{
      ...
      description: Faker.Lorem.sentences(1..3) |> Enum.join(" "),
      current_streak: Faker.random_between(0, 30),
      longest_streak: Faker.random_between(0, 100),
      habit_type:
        Enum.random([:basic_human_function, :self_improvement_theater, :actually_useful]),
      frequency: Enum.random([:daily, :weekly, :monthly]),
      user: build(:user)
    }
  end

  def habit_entry_factory do
    %HabitEntry{
      completed_on: Date.utc_today() |> Date.add(-Faker.random_between(0, 30)),
      # existential_commentary will be auto-generated by changeset
      habit: build(:habit)
    }
  end

  # Habit type variants
  def basic_human_function_habit_factory do
    build(:habit,
      habit_type: :basic_human_function,
      name: Enum.random(["Brush teeth", "Eat food", "Sleep", "Breathe consciously"]),
      description: "An essential activity that keeps you alive and functional"
    )
  end

  def self_improvement_theater_habit_factory do
    build(:habit,
      habit_type: :self_improvement_theater,
      name: Enum.random(["Meditate", "Journal", "Read self-help", "Wake up at 5 AM"]),
      description:
        "Activities that feel productive but mostly exist to make you feel better about yourself"
    )
  end

  def actually_useful_habit_factory do
    build(:habit,
      habit_type: :actually_useful,
      name: Enum.random(["Exercise regularly", "Learn a skill", "Save money", "Call family"]),
      description: "Genuinely beneficial practices with measurable positive impact"
    )
  end

  # Frequency variants
  def daily_habit_factory do
    build(:habit, frequency: :daily, name: "Daily practice")
  end

  def weekly_habit_factory do
    build(:habit, frequency: :weekly, name: "Weekly goal")
  end

  def monthly_habit_factory do
    build(:habit, frequency: :monthly, name: "Monthly objective")
  end

  # Streak variants
  def active_streak_habit_factory do
    current = Faker.random_between(5, 20)

    build(:habit,
      current_streak: current,
      longest_streak: max(current, Faker.random_between(current, current + 30))
    )
  end

  def broken_streak_habit_factory do
    build(:habit,
      current_streak: 0,
      longest_streak: Faker.random_between(10, 50)
    )
  end

  def perfect_habit_factory do
    streak = Faker.random_between(50, 365)

    build(:habit,
      current_streak: streak,
      longest_streak: streak,
      name: "Perfect consistency habit"
    )
  end

  # Habit entry variants
  def recent_habit_entry_factory do
    build(:habit_entry,
      completed_on: Date.utc_today() |> Date.add(-Faker.random_between(0, 3))
    )
  end

  def old_habit_entry_factory do
    build(:habit_entry,
      completed_on: Date.utc_today() |> Date.add(-Faker.random_between(30, 365))
    )
  end

  def today_habit_entry_factory do
    build(:habit_entry, completed_on: Date.utc_today())
  end

  def yesterday_habit_entry_factory do
    build(:habit_entry, completed_on: Date.utc_today() |> Date.add(-1))
  end

  def custom_commentary_habit_entry_factory do
    build(:habit_entry,
      existential_commentary: "Today I conquered the void of meaninglessness through routine"
    )
  end

  # Habit with entries for testing streaks
  def habit_with_entries_factory do
    habit = build(:habit)
    ...
    entries =
      for i <- 0..4 do
        build(:habit_entry,
          habit: habit,
          completed_on: Date.utc_today() |> Date.add(-i)
        )
      end

    %{habit | habit_entries: entries}
  end

  def habit_with_broken_streak_factory do
    habit = build(:habit)

    # Create entries with a gap (broken streak)
    entries = [
      build(:habit_entry, habit: habit, completed_on: Date.utc_today()),
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-1)),
      # Missing day -2
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-3)),
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-4))
    ]

    %{habit | habit_entries: entries}
  end

  def habit_with_long_history_factory do
    habit = build(:habit)

    # Create entries spanning multiple months with some gaps
    entries = [
      # Current streak (last 3 days)
      build(:habit_entry, habit: habit, completed_on: Date.utc_today()),
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-1)),
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-2)),

      # Gap

      # Previous streak (7 days ago to 10 days ago - 4 consecutive days)  
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-7)),
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-8)),
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-9)),
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-10)),

      # Older scattered entries
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-15)),
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-20)),
      build(:habit_entry, habit: habit, completed_on: Date.utc_today() |> Date.add(-30))
    ]
