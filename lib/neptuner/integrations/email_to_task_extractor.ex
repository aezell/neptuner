defmodule Neptuner.Integrations.EmailToTaskExtractor do
  ...
  def extract_tasks_from_email(email_summary) do
    analysis =
      AdvancedEmailAnalysis.analyze_email_advanced(%{
    ...
    potential_tasks =
      potential_tasks ++ extract_from_action_items(analysis.action_items, email_summary)

    # Extract from meeting potential
    potential_tasks =
      potential_tasks ++ extract_from_meeting_potential(analysis.meeting_potential, email_summary)
  ...
  def auto_convert_email_to_tasks(user_id, email_summary, opts \\ []) do
    ...
    tasks_to_create =
      ...
      # Limit to prevent task explosion
      |> Enum.take(opts[:max_auto_tasks] || 3)
  ...
  defp extract_from_meeting_potential(meeting_potential, email_summary) do
    if meeting_potential.confidence_score > 60 do
      ...
      meeting_tasks =
        if meeting_potential.meeting_type in [:presentation, :demo] do
          prep_task = %{
  ...
  defp calculate_action_confidence(action_text, action_items) do
    ...
    verb_boost =
      if String.match?(
           action_text,
           ~r/\b(send|create|update|review|approve|complete|finish|call|email|schedule|book|cancel|confirm)\b/i
         ) do
        15
      else
        0
    ...
    priority_boost =
      if action_items.priority_indicators > 0 do
        10
      else
        0
      end
    ...
    deadline_boost =
      if action_items.has_deadlines do
        10
      else
        0
      end
  ...
  defp estimate_action_importance(action_text, email_summary) do
    ...
    decision_boost =
      if String.match?(action_text, ~r/\b(approve|decide|choose|authorize|sign|confirm)\b/i) do
        15
      else
        0
      end
    ...
    creation_boost =
      if String.match?(action_text, ~r/\b(create|build|develop|complete|finish|deliver)\b/i) do
        10
      else
        0
      end
  ...
  defp calculate_existential_weight(task, email_summary) do
    ...
    creation_boost =
      if String.contains?(String.downcase(task.title), ["create", "build", "decide", "approve"]) do
        20
      else
        0
      end
    ...
    routine_penalty =
      if String.contains?(String.downcase(task.title), ["send", "reply", "forward", "update"]) do
        -15
      else
        0
      end
  ...
  defp build_task_description(task_data, email_summary) do
    ...
    metadata =
      if task_data[:cosmic_analysis] do
        metadata ++
          [
  ...
  defp generate_conversion_wisdom(potential_tasks, created_tasks) do
    ...
    %{
      ...
      task_stats: %{
        ...
        conversion_rate:
          if(potential_count > 0, do: created_count / potential_count * 100, else: 0)
      }
    }
  end
  ...
  defp analyze_common_task_patterns(emails) do
    ...
    %{
      ...
      action_classifications:
        Enum.frequencies(Enum.map(high_importance_emails, & &1.classification))
    }
  end
  ...
  defp extract_keywords(subject) do
    ...
    |> Enum.reject(
      &(&1 in ["with", "from", "this", "that", "will", "have", "been", "were", "they", "them"])
    )
  end
  ...
  defp generate_pattern_insights(emails) do
    ...
    actionable_emails =
      Enum.count(emails, &(&1.classification in [:urgent_important, :not_urgent_important]))
